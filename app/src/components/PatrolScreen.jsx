...
export default function PatrolScreen({ user, site, shift }) {
  ...
  // Ping GPS to server periodically during active patrol
  const gpsCountRef = useRef(0);
  useEffect(() => {
    if (!session || !patrolStarted || !currentPos) return;
    gpsCountRef.current++;
    if (gpsCountRef.current % 10 === 0) {
      api.patrols.gps(session.id, currentPos.lat, currentPos.lng).catch(() => {});
    }
  }, [currentPos, session, patrolStarted]);

  // Load route + active session
  useEffect(() => {
    if (!site?.id) return;
    async function load() {
      setLoading(true);
  ...
  async function endPatrol() {
    try {
      if (session?.id) await api.patrols.endSession(session.id);
    } catch {}
    ...
    <div style={{display:'flex',flexDirection:'column',height:'100vh',background:'#0b1222',overflow:'hidden'}}>
      ...
      <div style={{background:'#0f1929',padding:'10px 14px',display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:'1px solid rgba(255,255,255,0.06)',flexShrink:0}}>
        ...
        <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
          {currentPos && (
            <div style={{background:'rgba(0,0,0,0.4)',borderRadius:'999px',padding:'3px 8px',fontSize:'10px',color:'rgba(255,255,255,0.5)'}}>
                ...
                {checkpoints.map(cp => {
                  ...
                  return (
                    <div key={cp.id} onClick={() => canMark && markCheckpoint(cp)}
                      ...
                      {done && <div style={{fontSize:'10px',color:'rgba(74,222,128,0.5)',flexShrink:0}}>✓</div>}
                    </div>
                  );
                })}
              </div>
      ...
      {showCheckpointModal && (
        <CheckpointModal
          site={site} session={session} currentPos={currentPos} route={route}
          isRoutePlanner={isRoutePlanner}
          onClose={() => setShowCheckpointModal(false)}
          onSaved={() => {
            setShowCheckpointModal(false);
            setCheckpointSaving(true);
            setTimeout(() => setCheckpointSaving(false), 3000);
          }}
        />
      )}
  ...
  async function save() {
    ...
    try {
      ...
      await api.logs.create({
        ...
        log_type: 'PATROL',
        title: name.trim(),
        description: description.trim() || `Checkpoint: ${name.trim()}`,
        occurred_at: new Date().toISOString(),
        latitude: currentPos?.lat,
        longitude: currentPos?.lng,
        type_data: { checkpoint: true, patrol_session_id: session?.id },
      });
      // Save as permanent checkpoint if toggled
      if (savePermanent && route?.id && currentPos) {
        try {
          const existing = route.checkpoints || [];
          const newCp = { lat: currentPos.lat, lng: currentPos.lng, name: name.trim(), order_index: existing.length };
          await api.patrols.updateRoute(route.id, route.name, route.instructions || '', [...existing, newCp]);
      ...
      <div style={{background:'#0f1929',borderRadius:'16px 16px 0 0',padding:'0 0 20px',width:'100%',border:'1px solid rgba(255,255,255,0.08)'}}>
        ...
        <div style={{padding:'14px 16px'}}>
          ...
          <div style={{marginBottom:'12px'}}>
            ...
            <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Rear Gate, Car Park North"
          ...
          <div style={{marginBottom:'14px'}}>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)} rows={2} placeholder="What was checked or found..."
          ...
          {isRoutePlanner && route?.id && currentPos && (
            <div onClick={() => setSavePermanent(p => !p)}
              style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'11px 13px',background:savePermanent?'rgba(167,139,250,0.08)':'rgba(255,255,255,0.03)',border:`1px solid ${savePermanent?'rgba(167,139,250,0.3)':'rgba(255,255,255,0.07)'}`,borderRadius:'10px',marginBottom:'14px',cursor:'pointer'}}>
              <div>
                <div style={{fontSize:'13px',fontWeight:600,color:'#fff'}}>Save as permanent checkpoint</div>
                <div style={{fontSize:'11px',color:'rgba(255,255,255,0.35)',marginTop:'1px'}}>Add to {route.name} for future patrols</div>
              </div>
              <div style={{width:'38px',height:'22px',background:savePermanent?'#a78bfa':'rgba(255,255,255,0.1)',borderRadius:'999px',position:'relative',transition:'background 0.2s',flexShrink:0}}>
                <div style={{position:'absolute',top:3,left:savePermanent?'auto':'3px',right:savePermanent?'3px':'auto',width:16,height:16,background:'#fff',borderRadius:'50%',transition:'all 0.2s'}} />
              </div>
            </div>
          )}
          <button onClick={save} disabled={saving || !name.trim()}
            style={{width:'100%',padding:'14px',background:'#1a52a8',border:'none',borderRadius:'10px',color:'#fff',fontSize:'14px',fontWeight:700,cursor:'pointer',opacity:saving||!name.trim()?0.5:1}}>
...
function ReportModal({ user, site, session, onClose }) {
  ...
  async function submit() {
    ...
    try {
      await api.logs.create({
        site_id: site?.id, log_type: type,
        title: `Patrol Report: ${types.find(t=>t.key===type)?.label}`,
        description: notes, client_reportable: clientReportable,
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',zIndex:9999,display:'flex',alignItems:'flex-end'}}>
      <div style={{background:'#0f1929',borderRadius:'16px 16px 0 0',padding:'0 0 20px',width:'100%',maxHeight:'90vh',overflowY:'auto',border:'1px solid rgba(255,255,255,0.08)'}}>
        ...
        <div style={{padding:'14px 16px'}}>
          ...
          <textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={3} placeholder="Describe what you found..."
          ...
          <button onClick={submit} disabled={saving} style={{width:'100%',padding:'14px',background:'#1a52a8',border:'none',borderRadius:'10px',color:'#fff',fontSize:'14px',fontWeight:700,cursor:'pointer',opacity:saving?0.7:1}}>
