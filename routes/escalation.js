...
router.post('/check-call', authenticate, async (req, res, next) => {
  try {
    ...
    await supabase.from('occurrence_logs').insert({
      ...
      title: `Check Call — ${officer.first_name} ${officer.last_name}`,
      description: `Officer check call at ${new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', timeZone: 'Europe/London' })}. All OK.`,
...
router.post('/missed-check', authenticate, async (req, res, next) => {
  try {
    ...
    await supabase.from('occurrence_logs').insert({
      ...
      officer_id: officer.id,
      log_type: 'WELFARE_CHECK',
      title: `⚠ Missed Check Call — ${officer.first_name} ${officer.last_name}`,
      description: `Officer check call overdue by ${minutes_overdue || '?'} minutes.`,
      occurred_at: new Date().toISOString(),
