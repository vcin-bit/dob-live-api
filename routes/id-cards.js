...
router.get('/:id/pdf', authenticate, async (req, res, next) => {
  try {
    ...
    const { data: card, error } = await supabase
      .from('officer_id_cards')
      .select('*, officer:users(first_name, last_name, employee_number, sia_licence_number, sia_licence_type, sia_expiry_date, role)')
      .eq('id', req.params.id)
      .eq('company_id', req.user.company_id)
      .single();
    ...
    // Role
    doc.fontSize(6).font('Helvetica').fillColor('rgba(255,255,255,0.6)')
      .text((card.officer?.role || 'OFFICER').replace('_', ' '), 10, 46, { width: W - 20, align: 'center' });

    // Divider
    doc.rect(20, 55, W - 40, 0.5).fill('rgba(255,255,255,0.2)');

    // Details - left column
    const detY = 62;
    doc.fontSize(5).font('Helvetica').fillColor('rgba(255,255,255,0.5)');
    ...
    doc.fontSize(5).font('Helvetica').fillColor('rgba(255,255,255,0.7)')
