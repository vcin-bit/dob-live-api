...
router.get('/', authenticate, async (req, res, next) => {
  try {
    ...
    let q = supabase.from('patrol_routes').select('*, checkpoints:patrol_checkpoints(*)').eq('company_id', req.user.company_id).order('name');
...
router.post('/', authenticate, requireRole('SUPER_ADMIN', 'COMPANY', 'OPS_MANAGER'), async (req, res, next) => {
  try {
    ...
    const { data: route, error } = await supabase.from('patrol_routes').insert({ company_id: req.user.company_id, site_id, name, instructions, created_by: req.user.id }).select().single();
    if (error) throw error;
    if (checkpoints.length > 0) {
      ...
      await supabase.from('patrol_checkpoints').insert(rows);
    }
...
router.put('/:id', authenticate, requireRole('SUPER_ADMIN', 'COMPANY', 'OPS_MANAGER'), async (req, res, next) => {
  try {
    ...
    const { data, error } = await supabase.from('patrol_routes').update({ name, instructions }).eq('id', req.params.id).eq('company_id', req.user.company_id).select().single();
    if (error) throw error;
    if (checkpoints) {
      await supabase.from('patrol_checkpoints').delete().eq('route_id', req.params.id);
      if (checkpoints.length > 0) {
...
router.delete('/:id', authenticate, requireRole('SUPER_ADMIN', 'COMPANY', 'OPS_MANAGER'), async (req, res, next) => {
  try {
    const { error } = await supabase.from('patrol_routes').delete().eq('id', req.params.id).eq('company_id', req.user.company_id);
    if (error) throw error;
